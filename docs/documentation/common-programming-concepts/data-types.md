...
Signed numbers are stored using [two’scomplement](https://en.wikipedia.org/wiki/Two%27s_complement)<!-- ignore -->
representation.
...
> as 256, _integer overflow_ will occur, which can result in one of two
...
also written `()`. The type is called the _unit type_ and the value is called
the _unit value_. Expressions implicitly return the unit value if they don’t
...
that _is_ allowed to grow or shrink in size. If you’re unsure whether to use an
