...
The first command created a new directory called _hello_cargo_. We’ve named
our project _hello_cargo_, and Cargo creates its files in a directory of the
same name.
...
<span class="caption">Listing 1-2: Contents of _Cargo.toml_ generated by `cargo new`</span>

This file is in the [_TOML_](https://toml.io)<!-- ignore --> (_Tom’s Obvious,
Minimal Language_) format, which is Cargo’s configuration format.
...
as they write their program to make sure it compiles. Then they run `cargo build` when they’re ready to use the executable.
