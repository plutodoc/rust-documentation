...
into the scope of every program. This set is called the _prelude_, and you can
...
crate is a _library crate_, which contains code intended to be used in other
...
file. Because the _Cargo.lock_ file is important for reproducible builds, it's
...
the `0.9.x` series, you’d have to update the _Cargo.toml_ file to look like
this instead:
...
> using it. Another neat feature of Cargo is that running the `cargo doc --open` command will build documentation provided by all of your dependencies
...
a variable named `guess`? It does, but helpfully Rust allows us to _shadow_ the
...
method does (discussed earlier in [“Handling Potential Failure with the`Result` Type”](#handling-potential-failure-with-the-result-type)<!-- ignore
...
insatiable monster, as mentioned in the `parse` discussion in [“Comparing theGuess to the Secret Number”](#comparing-the-guess-to-the-secret-number)<!--
